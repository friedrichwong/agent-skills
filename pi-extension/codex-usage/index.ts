...
export function formatCodexUsageStatusline(
	...
	const weeklyLimit = snapshot.secondary ?? snapshot.primary;
	if (weeklyLimit) {
		const reset = compactReset(weeklyLimit.resetsAt);
		parts.push(`${formatRemainingPercent(weeklyLimit)} wk`);
		if (reset) parts.push(`→ ${reset}`);
	}
	if (!weeklyLimit && snapshot.credits) parts.push(formatCredits(snapshot.credits));
	return parts.join(" ");
}
...
function compactReset(epochSeconds?: number): string | undefined {
